...
local function closeCreator(cancel, creatingHouse)
    if not cancel and not creatingHouse then
        if zoneType == 'poly' then
...
local function startCreator(arg, useLast, creatingHouse)
    creatorActive = true
    controlsActive = true
    zoneType = arg
    ...
    while creatorActive do
        ...
        if controlsActive then
            ...
            elseif IsDisabledControlJustReleased(0, 201) then -- enter
                closeCreator(false, creatingHouse)
...
RegisterCommand('zone', function(source, args, rawCommand)
    ...
    startCreator(args[1], useLast, false)
end, true)
...
--             name = 'useLast',
--             help =
--             'duplicates the last created zone of the specified type (box and sphere only, optional)'
--         }
--     })
-- end)

function lib.zoneCreator()
    return startCreator("box", false, true)
end
